...
const (
	baseCommit             = "09dafe11c8113240ce106983e218f3290ab95ccb"
	baseTree               = "1ff82ffac9e8124cb384368afe3c3e06b6487494"
	contractRoot           = "CONTRACTS/tekroo.kernel.contracts/0.4.0"
	predecessorManifest    = "CONTRACTS/tekroo.kernel.contracts/0.3.0/manifest.json"
	predecessorManifestSHA = "a1831afd5829ce3d020cb65e51644416d6ec39b85dd4c00c82c9bef6f0294c2a"
	reportPath             = "OUTPUT/phase-3/step-9-escalation-contract-gate.json"
	focusPath              = "OUTPUT/phase-3/step-9-escalation-contract-tests.jsonl"
	fullPath               = "OUTPUT/phase-3/step-9-full-regression-tests.jsonl"
	structurePath          = "OUTPUT/phase-3/step-9-contract-structure.json"
	referencePath          = "OUTPUT/phase-3/step-9-contract-reference.json"
)
...
func main() {
	...
	if len(os.Args) != 1 {
		fatal(errors.New("usage: escalation-contract-report | -verify path"))
	}
	if err := run(); err != nil {
		fatal(err)
	}
...
func run() error {
	...
	paths := []string{
		...
		"OUTPUT/phase-3/step-8-acceptance.json",
		"OUTPUT/phase-3/step-8-release-receipt.json",
		"OUTPUT/phase-3/step-9-authorization.json",
		"OUTPUT/phase-3/step-9-contract-revision-authorization.json",
		"OUTPUT/phase-3/step-9-contract-encoding-gaps.md",
		"OUTPUT/phase-3/step-8-completion-reopening-gate.json",
		"CONTRACTS/tekroo.kernel.contracts/0.4.0/compatibility/from-0.3.0.json",
		"docs/architecture/010-deterministic-escalation-contract-revision.md",
	}
	...
	value := report{
		SchemaVersion: "1.0.0", ReportType: "PHASE_3_ESCALATION_CONTRACT_REVISION", Status: "PASS",
		...
		Assertions: []assertion{
			{"ESC-CONTRACT-001-DEDICATED-TYPES", "PASS", []string{"TestFrozenContractCorpus/CAT-028-ESCALATION-OPEN-VALID", "TestFrozenContractCorpus/CAT-029-ESCALATION-RESOLVE-VALID"}, "Escalation is a dedicated aggregate with explicit open/opened and resolve/resolved types; block and handoff are not aliases."},
			{"ESC-CONTRACT-002-EXACT-BOUNDS", "PASS", []string{"TestFrozenContractCorpus/ESCALATION-EXACT-ADJUDICATOR", "TestFrozenContractCorpus/ESCALATION-WRONG-ADJUDICATOR", "TestFrozenContractCorpus/ESCALATION-STALE-LIFECYCLE", "TestFrozenContractCorpus/ESCALATION-ROUND-EXHAUSTED"}, "Resolution binds the exact adjudicator, current work lifecycle epoch, and finite resolution-round budget."},
			{"ESC-CONTRACT-003-TIMEOUT", "PASS", []string{"TestFrozenContractCorpus/ESCALATION-ADJUDICATOR-DEADLINE", "TestFrozenContractCorpus/ESCALATION-POLICY-TIMEOUT", "TestFrozenContractCorpus/ESCALATION-TIMEOUT-BEFORE-DEADLINE", "TestFrozenContractCorpus/ESCALATION-TIMEOUT-OUTCOME"}, "After deadline only the exact timeout policy may terminate, and only as BLOCKED or HUMAN_REQUIRED."},
			{"ESC-CONTRACT-004-ONE-WAY", "PASS", []string{"TestFrozenContractCorpus/ESCALATION-ROUTE-ONCE", "TestFrozenContractCorpus/ESCALATION-NO-REROUTE", "TestFrozenContractCorpus/ESCALATION-TERMINAL", "TestTerminalEscalationCannotBeReopened"}, "An escalation routes once and cannot bounce, reroute, or reopen after a terminal decision."},
			{"ESC-CONTRACT-005-COMPATIBILITY", "PASS", []string{"TestFrozenContractCorpus", "TestCatalogueRejectsManifestIdentityMismatch", "TestCompletionCoordinatorStoryDependenciesAndReplayAreDeterministic"}, "Released 0.3.0 remains byte-identical; existing 1.2.0 semantics migrate identically to 1.3.0 and accepted Step 8 behavior remains passing."},
			{"ESC-CONTRACT-006-BOUNDARY", "PASS", []string{"TestFrozenCommandsApplyOrFailClosedAtUnqualifiedBoundary", "TestApplicationProductionImportsRemainKernelOnly"}, "The pure transition model is qualified, but authoritative escalation commands fail closed until coordination, snapshot projection, and repository integration are implemented."},
		},
		ExcludedProfiles: []excluded{
			{"escalation-coordination", "Automatic trigger selection, command planning, durable coordination, and repository integration remain the next implementation slice."},
			{"organizational-acceptance", "Escalation outcomes do not themselves establish organizational acceptance."},
