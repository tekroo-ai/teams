...
def _case(o: Mapping[str, Any]) -> bool:
    ...
    if case_id.endswith("FEEDBACK-LOOP-PREVENTION"):
        return len(requests) == len(terminals) and len(requests) in {1, 2} and len(o.get("hooks", [])) == 1 and len({row.get("id") for row in events}) == len(events)
    ...
    if case_id.endswith("CONDENSATION-REANCHOR"):
        summaries = [row for row in events if row.get("kind") == "Condensation" and row.get("summary")]
        prompt = next(
            row.get("prompt")
            for row in requests
