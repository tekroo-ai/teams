...
def handler(upstream: str, request_journal: Path, response_journal: Path):
    class AuditHandler(BaseHTTPRequestHandler):
        ...
        def _forward(self, body: bytes) -> None:
            ...
            if not operation:
                self.send_error(400, "missing X-SMA-E1-Operation")
                return
            request_id = sha256(f"{operation}:{started}:{threading.get_ident()}".encode()).hexdigest()
            append(request_journal, {
                "recordType":"SMA_E1_MODEL_AUDIT_REQUEST","requestId":request_id,
                "operationKey":operation,"method":self.command,"path":self.path,
                "receivedMonotonicNs":started,"requestBodyLength":len(body),
                "requestBodySha256":sha256(body).hexdigest(),
                "requestBodyBase64":base64.b64encode(body).decode("ascii"),
            })
            headers = {key:value for key,value in self.headers.items() if key.lower() not in {"host","content-length","x-sma-e1-operation"}}
            wire = urllib.request.Request(upstream_url(upstream, self.path), data=body if self.command != "GET" else None, headers=headers, method=self.command)
            error_kind = None
            try:
                ...
                response_body = canonical({"error":{"type":type(exc).__name__,"message":"upstream model transport failure"}})
                error_kind = type(exc).__name__
            completed = time.monotonic_ns()
            append(response_journal, {
                "recordType":"SMA_E1_MODEL_AUDIT_RESPONSE","requestId":request_id,
                "operationKey":operation,"completedMonotonicNs":completed,
                "httpStatus":status,"responseBodyLength":len(response_body),
                "responseBodySha256":sha256(response_body).hexdigest(),
                "responseBodyBase64":base64.b64encode(response_body).decode("ascii"),
                "responseWriteOutcome":"CLIENT_RECEIVED","upstreamErrorKind":error_kind,
            })
...
def main() -> int:
    ...
    server = ThreadingHTTPServer((args.host, args.port), handler(args.upstream, request_journal, response_journal))
    ready = Path(args.ready_file)
    descriptor = os.open(ready, os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o600)
    with os.fdopen(descriptor, "wb") as stream:
        stream.write(canonical({"pid":os.getpid(),"host":args.host,"port":args.port,"upstream":args.upstream}))
        stream.flush()
