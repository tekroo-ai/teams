...
from e1_native_v5.runner import NativeToolRunnerV5  # noqa: E402
...
def compose(repo: Path, mode: RunMode, authority: LiveAuthority, definition_path: Path = LAUNCH):
    require(mode == RunMode.ZERO_CREDIT_DRESS, FailureClass.SAFETY, "v5 continuation is zero-credit only")
    ...
    require(live_acceptance.get("status") == "ACCEPTED_FROZEN" and live_acceptance.get("packageIdentity") == identity.get("packageIdentity"), FailureClass.SAFETY, "v5 live acceptance mismatch")
    ...
    return NativeToolRunnerV5(ProductTruth.load(repo), LocalRawPorts(live, authority, mode), config, mode), tuple(OperationKey(str(row["s2Id"]), 1) for row in E1_CASES)
