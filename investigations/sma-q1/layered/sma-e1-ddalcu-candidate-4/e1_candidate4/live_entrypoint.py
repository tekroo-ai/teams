"""Default-deny E1 candidate-4 composition."""
...
from e1_candidate4.runner import Candidate4Runner  # noqa: E402
from t1.model import FailureClass, HarnessFailure, LiveAuthority, OperationKey, RunMode, file_sha256, require  # noqa: E402
...
def tree_sha(root: Path) -> str:
    entries = [[str(path.relative_to(root)), file_sha256(path)] for path in sorted(root.rglob("*")) if path.is_file() and "__pycache__" not in path.parts and path.suffix != ".pyc"]
...
def load_definition(path: Path = DEFINITION) -> Mapping[str, Any]:
    ...
    require(value.get("recordType") == "SMA_E1_DDALCU_CANDIDATE_4_LIVE_LAUNCH_DEFINITION", FailureClass.SAFETY, "wrong E1 candidate-4 live definition")
    require(value.get("status") == "PREPARED_NONLIVE_REQUIRES_ACCEPTANCE_EXECUTION_IDENTITY_AND_SINGLE_USE_AUTHORITY", FailureClass.SAFETY, "E1 candidate-4 definition is not default-deny")
    require(value.get("authority") == {"defaultDeny": True, "acceptedPackageRequired": True, "singleUseRequired": True, "modeExact": True, "consumeBeforeFirstRawCall": True}, FailureClass.SAFETY, "E1 candidate-4 authority policy drift")
    for binding in value.get("boundFiles", []):
        file = Path(binding["path"])
        require(file.is_file() and file_sha256(file) == binding["sha256"], FailureClass.SAFETY, f"candidate-4 bound file drift: {file}")
    for binding in value.get("boundTrees", []):
        root = Path(binding["path"])
        require(root.is_dir() and tree_sha(root) == binding["sha256"], FailureClass.SAFETY, f"candidate-4 bound tree drift: {root}")
    return value


def compose(repo: Path, mode: RunMode, authority: LiveAuthority, definition_path: Path = DEFINITION):
    require(mode in {RunMode.ZERO_CREDIT_DRESS, RunMode.MEASURED}, FailureClass.SAFETY, "candidate-4 live entrypoint requires a live mode")
    ...
    runner = Candidate4Runner(ProductTruth.load(repo), LocalRawPorts(live, authority, mode), config, mode)
...
def main() -> int:
    ...
    if not args.validate_only:
        raise HarnessFailure(FailureClass.SAFETY, "candidate-4 live execution requires separately frozen LiveAuthority")
    value = load_definition(Path(args.definition).resolve())
    print(json.dumps({"status": "PASS_DEFAULT_DENY_E1_CANDIDATE_4_COMPOSITION", "actions": len(value["actions"]), "liveExecution": False}, sort_keys=True))
    return 0
