...
CANDIDATE_6_PACKAGE = ROOT / "investigations/sma-q1/layered/sma-e1-ddalcu-candidate-6"
for package in (
    S2_PACKAGE,
    CANDIDATE_1_PACKAGE,
    CANDIDATE_3_PACKAGE,
...
from e1_candidate6.runner import Candidate6Runner  # noqa: E402
...
def load_definition(path: Path = DEFINITION) -> Mapping[str, Any]:
    ...
    require(
        ...
        == "SMA_E1_DDALCU_CANDIDATE_6_LIVE_LAUNCH_DEFINITION",
        FailureClass.SAFETY,
        "wrong E1 candidate-6 live definition",
    )
    require(
        value.get("status")
        == "PREPARED_NONLIVE_REQUIRES_ACCEPTANCE_EXECUTION_IDENTITY_AND_SINGLE_USE_AUTHORITY",
        FailureClass.SAFETY,
        "E1 candidate-6 definition is not default-deny",
    )
    require(
        value.get("authority")
        == {
        ...
        FailureClass.SAFETY,
        "E1 candidate-6 authority policy drift",
    )
    require(
        Path(value.get("modelAuditProxyPath", "")).resolve()
        == Path(__file__).with_name("model_audit_proxy.py").resolve(),
        FailureClass.SAFETY,
        "candidate-6 interaction-bound model proxy drift",
    )
    for binding in value.get("boundFiles", []):
        ...
        require(
            ...
            FailureClass.SAFETY,
            f"candidate-6 bound file drift: {file}",
        )
    for binding in value.get("boundTrees", []):
        ...
        require(
            ...
            FailureClass.SAFETY,
            f"candidate-6 bound tree drift: {root}",
        )
    return value
...
def compose(
    ...
    require(
        ...
        FailureClass.SAFETY,
        "candidate-6 live entrypoint requires a live mode",
    )
...
def main() -> int:
    ...
    if not args.validate_only:
        raise HarnessFailure(
            FailureClass.SAFETY,
            "candidate-6 live execution requires separately frozen LiveAuthority",
        )
    value = load_definition(Path(args.definition).resolve())
    print(json.dumps({
        "status": "PASS_DEFAULT_DENY_E1_CANDIDATE_6_COMPOSITION",
