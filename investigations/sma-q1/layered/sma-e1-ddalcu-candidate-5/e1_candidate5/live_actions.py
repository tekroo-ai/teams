#!/usr/bin/env python3
"""Self-contained candidate-5 live-action launcher."""
...
def load_definition(path: Path) -> Mapping[str, Any]:
    ...
    if value.get("recordType") != "SMA_E1_DDALCU_CANDIDATE_5_LIVE_LAUNCH_DEFINITION":
        raise ValueError("wrong E1 candidate-5 launch definition")
    if set(value.get("actions", [])) != ACTIONS:
        raise ValueError("incomplete E1 candidate-5 action set")
    for binding in value.get("boundFiles", []):
        file = Path(binding["path"])
        if not file.is_file() or s2._sha(file) != binding["sha256"]:
            raise ValueError(f"bound E1 candidate-5 file drift: {file}")
    for binding in value.get("boundTrees", []):
        root = Path(binding["path"])
        if not root.is_dir() or s2._tree_sha(root) != binding["sha256"]:
            raise ValueError(f"bound E1 candidate-5 tree drift: {root}")
    exact = {
    ...
    if any(
        ...
        raise ValueError("E1 candidate-5 model profile drift")
    return value
