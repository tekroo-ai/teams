...
from t1.model import FailureClass, LiveAuthority, RunMode, file_sha256, require  # noqa: E402
...
def compose(repo: Path, mode: RunMode, authority: LiveAuthority, definition_path: Path = LAUNCH):
    require(mode == RunMode.MEASURED, FailureClass.SAFETY, "final E1 package is measured-only")
    ...
    require(
        accepted.get("status") == "ACCEPTED_FROZEN"
        and accepted.get("packageIdentity") == identity.get("packageIdentity"),
        FailureClass.SAFETY,
        "final E1 live acceptance mismatch",
    )
    authority = replace(
        authority,
    ...
    runner = NativeToolRunnerV9(ProductTruth.load(repo), LocalRawPorts(live, authority, mode), config, mode)
    plan = e1_plan()
    require(len(E1_CASES) == 18 and len(plan) == 96, FailureClass.HARNESS, "final E1 plan cardinality drift")
    return runner, plan
