...
func TestAllFrozenCommandsReachTheirDeclaredEventThroughEvaluator(t *testing.T) {
	...
	fixtureBytes, err := os.ReadFile(filepath.Join(root, "CONTRACTS/tekroo.kernel.contracts/0.12.0/fixtures/catalogue-coverage.json"))
	if err != nil {
		t.Fatal(err)
	}
	catalogueBytes, err := os.ReadFile(filepath.Join(root, "CONTRACTS/tekroo.kernel.contracts/0.12.0/catalogue/kernel-catalogue.json"))
	if err != nil {
		t.Fatal(err)
	}
