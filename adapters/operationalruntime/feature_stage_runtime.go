...
func planningStageDefinition(stage featurePlanningStage) (string, kernel.WorkPurpose, string, string, []string, error) {
	...
	case stageSpecification:
		return "project-manager", kernel.PurposeHandoff, "Specify feature stories", "Analyze only the authoritative feature state below. Return FEATURE_SPECIFICATION JSON with schema_version, result_type, stories (title, description, acceptance_criteria, priority), and design_constraints. Every priority must be exactly one of LOW, NORMAL, HIGH, or CRITICAL. Do not add or change product requirements to name an actor, actor instance, branch, worktree, workspace path, model profile, execution identity, or fencing epoch; Teams assigns operational identities only after the product definition is validated. This is a reasoning-only task: do not use terminal, browser, repository, or other tools. Do not infer a different feature from workspace contents. Do not delegate or start another agent. Call the OpenHands finish tool exactly once. Put TEKROO_ORGANIZATIONAL_RESULT: on its own line followed by exactly one FEATURE_SPECIFICATION JSON object in finish.message. finish.message is the only result Teams receives. Do not answer with an assistant text message, summary, explanation, Markdown fence, XML, or trailing content. A prose assessment is not a result and will be rejected.", []string{"stories are finite, testable, and within the accepted feature scope"}, nil
