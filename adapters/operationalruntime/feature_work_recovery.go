...
func (service *ProductionService) reconcileFeaturePlan(ctx context.Context, feature organization.FeatureRequest, plan organization.FeaturePlan) error {
	...
	terminalRetried, err := service.reconcileRetryableTerminatedTasks(ctx, feature, plan, states, invocations)
	if err != nil {
		return fmt.Errorf("reconcile retryable terminations: %w", err)
	}
	if terminalRetried {
		return nil
	}
