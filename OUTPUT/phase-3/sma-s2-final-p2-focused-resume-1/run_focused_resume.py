...
SELECTED = {
    ...
    ("SMA-S2-019-MODEL-STUB-FAULT-MATRIX", 1),
    ("SMA-S2-019-MODEL-STUB-FAULT-MATRIX", 3),
...
EXECUTION_IDENTITY = "2fe93e26dcfc7a47f611b86d12c064c9f6d93c7f195a9315f905d4ff2e46fd45"
...
def main() -> int:
    ...
    plan = tuple(
        item
        for item in complete_plan
        if (item.case_id, item.repetition) in SELECTED
    )
    if len(plan) != 7:
        raise RuntimeError(f"expected seven focused operations, found {len(plan)}")
    ...
    receipt = {
        ...
        "checkpoint": "POST_RUN_7_FOCUSED_REMEDIATION",
        "operationsExpected": 7,
