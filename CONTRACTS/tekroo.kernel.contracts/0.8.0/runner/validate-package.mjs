...
check("contract-identity", manifest.contract.name === "tekroo.kernel.contracts" && manifest.contract.version === "0.8.0");
...
check("decision-trace-count", decisions.length === 50, decisions.length);
check("negative-requirement-trace-count", negatives.length === 26, negatives.length);
check("prohibition-trace-nonempty", prohibitions.length > 0, prohibitions.length);

const compatibility = readJson("compatibility/from-0.7.0.json");
check("successor-compatibility-record", compatibility.predecessor?.contractVersion === "0.7.0" && compatibility.contractVersion === "0.8.0" && compatibility.directions?.adapter === "REQUIRED");
