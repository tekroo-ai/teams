...
SOURCE_LAUNCH = ROOT / "investigations/sma-q1/layered/sma-e1-ddalcu-candidate-5/e1_candidate5/live-launch-definition.json"
PACKAGE_ROOT = ROOT / "investigations/sma-q1/layered/sma-e1-ddalcu-candidate-6/e1_candidate6"
LAUNCH = PACKAGE_ROOT / "live-launch-definition.json"
QUALIFICATION = ROOT / "OUTPUT/phase-3/sma-e1-ddalcu-candidate-6-offline-qualification/offline-qualification.json"
PACKAGE_IDENTITY = ROOT / "investigations/sma-q1/layered/sma-e1-ddalcu-candidate-6-package-identity.json"
...
def build_launch() -> None:
    if LAUNCH.exists():
        raise RuntimeError("candidate-6 launch definition already exists")
    value = json.loads(SOURCE_LAUNCH.read_text(encoding="utf-8"))
    value["recordType"] = "SMA_E1_DDALCU_CANDIDATE_6_LIVE_LAUNCH_DEFINITION"
    value["disposable"].update({
        "mongoDatabase": "sma_e1_ddalcu_candidate_6",
        "semanticCollection": "sma_e1_ddalcu_candidate_6_semantic",
        "episodicCollection": "sma_e1_ddalcu_candidate_6_episodic",
        "workspaceRoot": "/Users/paul/work/tekroo-ai/sma-step15-s1-p2final/target/sma-e1-ddalcu-candidate-6",
        "launchLabel": "com.tekroo.sma-service-e1-ddalcu-candidate-6",
        "stubLaunchLabel": "com.tekroo.sma-e1-model-audit-proxy-candidate-6",
    })
    runtime = "/Users/paul/work/tekroo-ai/sma-step15-s1-p2final/target/sma-e1-ddalcu-candidate-6/runtime"
    value["evidenceFiles"] = {
    ...
    value["modelAuditProxyPath"] = str(PACKAGE_ROOT / "model_audit_proxy.py")
    value["agentLoopAudit"].update({
        "finalFinishToolMessageRequiredPerUserPrompt": False,
        "plainFinalResponsePermittedByAcceptedScience": True,
        "finalResponseSelection": "ONE_FINAL_RESPONSE_PER_EXACT_CONVERSATION_AND_USER_TURN",
        "modelCallsBoundToConversationRole": True,
        "condensationCallExcludedFromUserAnswers": True,
        "forbiddenMarkersCheckedAcrossAllModelOutputs": True,
        "intermediateReasoningCannotSatisfyModelPredicate": True,
        "oneRepetitionPerScenarioRealPathCanaryRequired": True,
    })
    additions = [
        PACKAGE_ROOT / "__init__.py",
        PACKAGE_ROOT / "runner.py",
        PACKAGE_ROOT / "authority.py",
        PACKAGE_ROOT / "model_audit_proxy.py",
        PACKAGE_ROOT / "live_actions.py",
        PACKAGE_ROOT / "live_entrypoint.py",
        ROOT / "scripts/build_sma_e1_ddalcu_candidate_6.py",
        ROOT / "scripts/qualify_sma_e1_ddalcu_candidate_6.py",
        ROOT / "scripts/execute_sma_e1_ddalcu_candidate_6.py",
    ]
...
def build_identity() -> None:
    if PACKAGE_IDENTITY.exists():
        raise RuntimeError("candidate-6 package identity already exists")
    ...
    for binding in launch["boundFiles"]:
        ...
        if not path.is_file() or sha(path) != binding["sha256"]:
            raise RuntimeError(f"candidate-6 bound file drift: {path}")
        files.append({
            "path": str(path.relative_to(ROOT)) if path.is_relative_to(ROOT) else str(path),
            "sha256": binding["sha256"],
        })
    subject = {
        "lineage": "SMA-E1-DDALCU-CANDIDATE-6",
    ...
    value = {
        "schemaVersion": "1.0.0",
        "recordType": "SMA_E1_DDALCU_CANDIDATE_6_PACKAGE_IDENTITY",
