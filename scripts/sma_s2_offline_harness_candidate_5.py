...
def assert_client_disconnect_control(
    port: int,
    raw: Path,
    terminal: Path,
) -> dict[str, Any]:
    payload = canonical_json_bytes(fixture("OFFLINE-CANCELLATION", 8))
    request = (
        b"POST /v1/chat/completions HTTP/1.1\r\n"
        b"Host: 127.0.0.1\r\n"
        b"Content-Type: application/json\r\n"
        b"X-SMA-S2-Mode: TIMEOUT\r\n"
    ...
    raw_deadline = time.monotonic() + 2.0
    request_id: str | None = None
    while time.monotonic() < raw_deadline:
        if raw.exists():
            matches = [
                record
                for record in read_json_lines(raw)
                if record["caseId"] == "OFFLINE-CANCELLATION"
            ]
            if matches:
                request_id = matches[-1]["requestId"]
                break
        time.sleep(0.01)
    ...
    while time.monotonic() < terminal_deadline:
        if terminal.exists():
            matches = [
                record
                for record in read_json_lines(terminal)
                if record["requestId"] == request_id
            ]
            if matches:
                terminal_record = matches[-1]
                break
        time.sleep(0.01)
    assert terminal_record is not None
    assert terminal_record["responseWriteOutcome"] == "CLIENT_DISCONNECTED"
    return {
        "requestId": request_id,
        "cancelledMonotonicNs": cancelled_ns,
        "completedMonotonicNs": terminal_record["completedMonotonicNs"],
        "responseWriteOutcome": terminal_record["responseWriteOutcome"],
        "completionAfterCancellationNs": (
            terminal_record["completedMonotonicNs"] - cancelled_ns
        ),
    }
...
def assert_journal_integrity(
    ...
    terminal_records = read_json_lines(terminal)
    assert len(raw_records) == len(operational_records)
    assert len(raw_records) == len(terminal_records)
    assert len(raw_records) >= 11
    ...
    terminal_by_id = {record["requestId"]: record for record in terminal_records}
    assert raw_by_id.keys() == op_by_id.keys() == terminal_by_id.keys()
    for request_id, raw_record in raw_by_id.items():
        ...
        assert terminal_by_id[request_id]["responseWriteOutcome"] in {
            "CLIENT_RECEIVED",
            "CLIENT_DISCONNECTED",
        }
    return {
        ...
        "terminalJournalSha256": sha256_file(terminal),
        "allRawBodiesReconstructed": True,
        "identitySetsEqual": True,
        "terminalOutcomeCount": len(terminal_records),
    }
...
def main() -> int:
    ...
    terminal = output / "stub-terminal.jsonl"
    ...
    try:
        with stdout_file.open("w") as stdout, stderr_file.open("w") as stderr:
            ...
            results.append(
                run_test(
                    "OFFLINE-005-CLIENT-DISCONNECT-CONTROL",
                    lambda: assert_client_disconnect_control(port, raw, terminal),
                )
            )
            results.append(
                run_test(
                    "OFFLINE-006-FOUR-CHANNEL-CONCURRENCY",
                    lambda: assert_concurrency(port),
                )
            )
            results.append(
                run_test(
                    "OFFLINE-007-SECRET-SURFACE-POLICY",
            ...
            results.append(
                run_test(
                    "OFFLINE-008-JOURNAL-RECONSTRUCTION",
                    lambda: assert_journal_integrity(raw, operational, terminal),
                )
            )
    finally:
