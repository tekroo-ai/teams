...
class DeterministicStubHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        if mode == "MALFORMED":
            ...
            outcome = self._send_bytes(
                HTTPStatus.OK, response, "application/json"
            )
            self._record_terminal(
                request_id=request_id,
                case_id=case_id,
        ...
        if request.get("stream") is True:
            chunks = [
                {
                    ...
                    "created": 0,
                    "model": "sma-s2-deterministic-stub-candidate-2",
                    "choices": [
                        {
                            "index": 0,
            ...
            outcome = self._send_bytes(
                HTTPStatus.OK, payload, "text/event-stream"
            )
            self._record_terminal(
                request_id=request_id,
                case_id=case_id,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--terminal-journal", type=Path, required=True)
