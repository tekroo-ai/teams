...
PREDECESSOR = ROOT / "investigations/sma-q1/layered/sma-e1-ddalcu-native-tool-repair-v4-package-identity.json"
...
IDENTITY = ROOT / "investigations/sma-q1/layered/sma-e1-ddalcu-native-tool-repair-v5-package-identity.json"
...
AUTHORITY = ROOT / "investigations/sma-q1/layered/sma-e1-ddalcu-native-tool-repair-v5-case18-authorization.json"
V5 = ROOT / "investigations/sma-q1/layered/sma-e1-ddalcu-native-tool-repair-v5"
SOURCES = (V5 / "e1_native_v5/__init__.py", V5 / "e1_native_v5/runner.py", V5 / "e1_native_v5/live_entrypoint.py", ROOT / "scripts/qualify_sma_e1_native_tool_repair_v5.py", ROOT / "scripts/execute_sma_e1_native_tool_repair_v5_case18.py", ROOT / "scripts/build_sma_e1_native_tool_repair_v5.py")
...
def identity() -> None:
    ...
    if digest(sources) != offline["packageSourceIdentity"]:
        raise RuntimeError("v5 source identity mismatch")
    subject = {"lineage": "SMA-E1-DDALCU-NATIVE-TOOL-REPAIR-V5", "predecessorPackageIdentity": predecessor["packageIdentity"], "changeScope": "WAIT_FOR_TURN_ONE_BEFORE_CONDENSATION_AND_BIND_NEW_TURN_TERMINAL", "packageSourceIdentity": offline["packageSourceIdentity"], "sources": sources, "launchDefinition": {"path": str(LAUNCH.relative_to(ROOT)), "sha256": sha(LAUNCH)}, "offlineQualification": {"path": str(OFFLINE.relative_to(ROOT)), "sha256": sha(OFFLINE), "receiptSha256": offline["receiptSha256"]}, "sourceCheckpoint": {"path": str(SOURCE_RECEIPT.relative_to(ROOT)), "sha256": sha(SOURCE_RECEIPT)}, "preservedPasses": 17, "continuationScenarios": 1, "scienceChanged": False, "modelProfileChanged": False, "measuredExecution": False}
    value = {"schemaVersion": "1.0.0", "recordType": "SMA_E1_DDALCU_NATIVE_TOOL_REPAIR_V5_PACKAGE_IDENTITY", "status": "PREPARED_NOT_MEASURED", "identitySubject": subject, "packageIdentity": digest(subject)}
...
def acceptances() -> None:
    ...
    write(ACCEPTANCE, dict(common, recordType="SMA_E1_DDALCU_NATIVE_TOOL_REPAIR_V5_ACCEPTANCE", status="ACCEPTED_FROZEN_FOR_ZERO_CREDIT_VALIDATION"))
    write(LIVE_ACCEPTANCE, dict(common, recordType="SMA_E1_DDALCU_NATIVE_TOOL_REPAIR_V5_LIVE_ACCEPTANCE", status="ACCEPTED_FROZEN"))
    print(json.dumps({"acceptanceSha256": sha(ACCEPTANCE), "liveAcceptanceSha256": sha(LIVE_ACCEPTANCE)}, sort_keys=True))


def authority() -> None:
    package = json.loads(IDENTITY.read_text(encoding="utf-8"))
    value = {"schemaVersion": "1.0.0", "recordType": "SMA_E1_DDALCU_NATIVE_TOOL_REPAIR_V5_PRINCIPAL_AUTHORIZATION", "status": "AUTHORIZED_BY_PRINCIPAL", "recordedAt": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"), "packageIdentity": package["packageIdentity"], "sourceV4ReceiptSha256": sha(SOURCE_RECEIPT), "scope": "SCENARIO_18_ZERO_CREDIT_CONTINUATION", "mode": "ZERO_CREDIT_DRESS", "singleUse": True, "maximumScenarios": 1, "maximumRepetitions": 1, "automaticReruns": 0, "measuredExecution": False, "principalStatement": PRINCIPAL_STATEMENT}
