...
const packageRoot = path.join(repoRoot, "CONTRACTS/tekroo.kernel.contracts/0.1.0");
...
if (fs.existsSync(manifestPath) || fs.existsSync(checksumPath)) {
  throw new Error("contract 0.1.0 is already sealed; refusing in-place regeneration");
}
...
const contractBase = "https://contracts.tekroo.ai/tekroo.kernel.contracts/0.1.0";
...
const operations = [
  ...
  operation({
    ...
    fields: { evidence_kind: string({ enum: ["SOURCE_SNAPSHOT", "SOURCE_DIFF", "TEST_RESULT", "TOOL_RESULT", "ARTIFACT", "PROVIDER_RECEIPT", "MODEL_OUTPUT", "HUMAN_ATTESTATION", "DECISION_RECORD", "EXTERNAL_OBSERVATION"] }), sha256: digest(), byte_length: integer({ minimum: 0 }), media_type: string({ minLength: 1, maxLength: 255 }), locator: nonempty() },
    example: { evidence_kind: "TEST_RESULT", sha256: "b".repeat(64), byte_length: 42, media_type: "application/json", locator: "artifact://tests/result-1" },
  }),
  operation({
  ...
  operation({
    ...
    targetKinds: ["task"], authorityKinds: ["ACTOR", "HUMAN", "POLICY"], executionRequired: true,
    fields: { criteria_revision: integer({ minimum: 1 }), evidence_ids: uuidList({ minItems: 1 }), artifact_digests: digestList(), unresolved_exceptions: stringList() },
    example: { criteria_revision: 1, evidence_ids: ["00000000-0000-7000-8000-000000000203"], artifact_digests: ["d".repeat(64)], unresolved_exceptions: [] },
  }),
  operation({
    command: "tekroo.command.story.request-completion",
    event: "tekroo.event.story.completed",
    targetKinds: ["story"], authorityKinds: ["ACTOR", "HUMAN", "POLICY"],
    fields: { criteria_revision: integer({ minimum: 1 }), evidence_ids: uuidList({ minItems: 1 }), validation_event_ids: uuidList({ minItems: 1 }) },
    example: { criteria_revision: 1, evidence_ids: ["00000000-0000-7000-8000-000000000204"], validation_event_ids: ["00000000-0000-7000-8000-000000000304"] },
  }),
  operation({
    command: "tekroo.command.story.request-acceptance",
    event: "tekroo.event.story.accepted",
    targetKinds: ["story"], authorityKinds: ["HUMAN", "POLICY"],
    fields: { acceptance_policy_revision: integer({ minimum: 1 }), evidence_ids: uuidList({ minItems: 1 }), qualified_tree_digest: digest() },
    optional: ["qualified_tree_digest"],
    example: { acceptance_policy_revision: 1, evidence_ids: ["00000000-0000-7000-8000-000000000205"], qualified_tree_digest: "e".repeat(64) },
  }),
  operation({
  ...
  operation({
    ...
    targetKinds: ["story", "task"], authorityKinds: ["HUMAN", "POLICY"],
    fields: { successor_id: uuid(), relation: string({ enum: ["DERIVATION", "SUPERSESSION", "SPLIT", "MERGE", "CORRECTION"] }), reason: nonempty() },
    example: { successor_id: "00000000-0000-7000-8000-000000000401", relation: "SUPERSESSION", reason: "scope replacement" },
  }),
  operation({
    ...
    fields: { subject_kind: string({ enum: ["story", "task"] }), subject_id: uuid(), lifecycle_epoch: integer({ minimum: 1 }), criteria_revision: integer({ minimum: 1 }), evidence_set_digest: digest() },
    example: { subject_kind: "task", subject_id: "00000000-0000-7000-8000-000000000101", lifecycle_epoch: 1, criteria_revision: 1, evidence_set_digest: "f".repeat(64) },
  }),
  operation({
    ...
    fields: { review_id: uuid(), result: string({ enum: ["PASS", "FAIL", "INCONCLUSIVE"] }), reasons: stringList(), evidence_ids: uuidList({ minItems: 1 }) },
    example: { review_id: "00000000-0000-7000-8000-000000000501", result: "PASS", reasons: [], evidence_ids: ["00000000-0000-7000-8000-000000000207"] },
  }),
  operation({
...
for (const op of operations) {
  const commandSchemaName = op.command.replaceAll(".", "_").replaceAll("-", "_").concat("_1_0_0");
  const eventSchemaName = op.event.replaceAll(".", "_").replaceAll("-", "_").concat("_1_0_0");
  ...
  catalogueEntries.push({
    ...
    emits: [op.event],
    aliases: [],
    compatibility: { acceptedSourceVersions: ["1.0.0"], transforms: [] },
  });
  catalogueEntries.push({
    typeId: op.event,
    kind: "EVENT",
...
const coreSchema = {
  ...
  $defs: {
    ...
    ContractManifestIdentity: { const: "tekroo.kernel.contracts/0.1.0" },
    AggregateRef: {
...
writeJson("schemas/kernel-command.schema.json", {
  ...
  properties: {
    ...
    expected_revision: { anyOf: [{ type: "integer", minimum: 1 }, { const: "MUST_NOT_EXIST" }] },
  ...
  required: ["contract_manifest", "command_id", "command_type", "command_version", "target", "authority", "actor_fqn", "execution", "expected_revision", "idempotency_key", "correlation_id", "causation", "issued_at", "payload", "evidence_refs"],
});
...
writeJson("schemas/evidence-record.schema.json", {
  ...
  required: ["evidence_id", "evidence_kind", "raw_sha256", "byte_length", "media_type", "locator", "availability", "registered_by", "source_evidence_ids", "sensitivity", "retention_policy", "integrity_state"],
});
...
writeJson("schemas/conformance-fixture.schema.json", {
  ...
  properties: {
    ...
    kind: { enum: ["CATALOGUE_COMMAND", "STATE_MODEL", "DAG_MODEL", "IDENTITY_MODEL", "IDEMPOTENCY_MODEL"] },
...
writeJson("schemas/conformance-report.schema.json", {
  ...
  properties: {
    schemaVersion: { const: "1.0.0" }, reportType: { type: "string", minLength: 1 }, contractIdentity: { const: "tekroo.kernel.contracts/0.1.0" },
...
for (const op of operations) {
  ...
  fixtures.push({
    ...
    given: { contractManifest: "tekroo.kernel.contracts/0.1.0" },
...
const idemScope = { contract: "tekroo.kernel.contracts/0.1.0", principal: "teams::coder-1", commandType: "tekroo.command.task.activate", target: "task-1", key: "activate-1" };
...
writeJson("fixtures/catalogue-coverage.json", { schemaVersion: "1.0.0", contractIdentity: "tekroo.kernel.contracts/0.1.0", fixtures: catalogueFixtures });
writeJson("fixtures/model-and-invariant-scenarios.json", { schemaVersion: "1.0.0", contractIdentity: "tekroo.kernel.contracts/0.1.0", fixtures: modelFixtures });

const invariants = [
  { invariantId: "INV-001-REVISION-CONTIGUITY", description: "Accepted transitions advance exact contiguous revisions; rejections do not.", decisionIds: ["P2-KCF-003", "P2-KCF-007"], negativeRequirementIds: [], fixtureIds: ["MODEL-TASK-FORWARD"] },
  ...
  { invariantId: "INV-006-LIFECYCLE-EPOCHS", description: "Completion, acceptance, reopening, and correction preserve immutable lifecycle epochs.", decisionIds: ["P2-KCF-006", "P2-KCF-008"], negativeRequirementIds: ["NEG-003", "NEG-008"], fixtureIds: ["MODEL-COMPLETION-NO-REQUESTED-PHASE", "MODEL-REOPEN-INCREMENTS-EPOCH", "MODEL-ILLEGAL-COMPLETION"] },
  ...
  { invariantId: "INV-012-CONFORMANCE-REPRODUCIBLE", description: "Canonical fixtures and reports reproduce under pinned deterministic inputs.", decisionIds: ["P2-KCF-011", "P2-KCF-012"], negativeRequirementIds: [], fixtureIds: ["MODEL-STORY-FORWARD", "DAG-VALID-BRANCH-JOIN"] },
...
writeJson("invariants/invariants.json", { schemaVersion: "1.0.0", contractIdentity: "tekroo.kernel.contracts/0.1.0", invariants });
...
const decisionTests = {
  ...
  "P2-KCF-007": ["INV-001-REVISION-CONTIGUITY", "INV-002-IDEMPOTENT-DECISION", "INV-007-BOUNDED-ITERATION", "IDEMPOTENCY-DUPLICATE-AND-CONFLICT"],
  "P2-KCF-008": ["INV-006-LIFECYCLE-EPOCHS", "MODEL-COMPLETION-NO-REQUESTED-PHASE", "MODEL-REOPEN-INCREMENTS-EPOCH"],
  ...
  "P2-KCF-012": ["INV-012-CONFORMANCE-REPRODUCIBLE", "INV-013-MERGE-TREE-QUALIFIED"],
};
...
writeJson("compatibility/no-predecessor.json", {
  schemaVersion: "1.0.0",
  contractName: "tekroo.kernel.contracts",
  contractVersion: "0.1.0",
  predecessor: null,
  compatibilityClaim: "INITIAL_BASELINE_NO_PREDECESSOR",
  historicalV3Compatibility: "NOT_CLAIMED",
  migrationAuthority: "NONE",
  rollback: "No predecessor package exists; rejection of this candidate leaves Phase 1B architecture as the accepted design baseline.",
});
...
function dependenciesFor(relativePath) {
  ...
  if (relativePath === "runner/validate-package.mjs") return ["catalogue/kernel-catalogue.json", "schemas/payloads.schema.json", "invariants/invariants.json", "traceability/traceability.json", "compatibility/no-predecessor.json", "runner/protocol.json"];
...
const manifest = {
  schemaVersion: "1.0.0",
  contract: { name: "tekroo.kernel.contracts", version: "0.1.0", status: "FROZEN_CANDIDATE" },
  ...
  sourceLineage: [
    ...
    { path: "scripts/generate_phase2_contract.mjs", sha256: sha256(read("scripts/generate_phase2_contract.mjs")) },
  ],
  ...
  knownLimitations: [
    ...
    "The payload catalogue is the 0.1.0 bootstrap surface and may require explicitly versioned additions before implementation scope expands.",
  ],
};
...
process.stdout.write(`GENERATED tekroo.kernel.contracts/0.1.0 ${manifestDigest} ${fileInventory.length}\n`);
