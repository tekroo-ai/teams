...
from e1_candidate3.live_entrypoint import compose  # noqa: E402
...
EXECUTION_IDENTITY = "e0aed2cbdc8b162d55ca34237c914ce8c77291eb3b9a06e98715e2eadbce3222"
...
IDENTITY_ACCEPTANCE = ROOT / "investigations/sma-q1/layered/sma-e1-ddalcu-candidate-3-execution-identity-acceptance.json"
PREFLIGHT = ROOT / "OUTPUT/phase-3/sma-e1-ddalcu-candidate-3-live-state-preflight/preflight.json"
AUTHORIZATION = ROOT / "investigations/sma-q1/layered/sma-e1-ddalcu-candidate-3-measured-execution-authorization.json"
OUTPUT = ROOT / "OUTPUT/phase-3/sma-e1-ddalcu-candidate-3-measured-execution"
...
def main() -> int:
    if OUTPUT.exists():
        raise RuntimeError("candidate-3 single-use measured output already exists")
    authorization = load(AUTHORIZATION, "SMA_E1_DDALCU_CANDIDATE_3_MEASURED_EXECUTION_AUTHORIZATION")
    package_acceptance = load(PACKAGE_ACCEPTANCE, "SMA_E1_DDALCU_CANDIDATE_3_ACCEPTANCE")
    identity = load(IDENTITY, "SMA_E1_DDALCU_CANDIDATE_3_EXECUTION_IDENTITY")
    identity_acceptance = load(IDENTITY_ACCEPTANCE, "SMA_E1_DDALCU_CANDIDATE_3_EXECUTION_IDENTITY_ACCEPTANCE")
    preflight = load(PREFLIGHT, "SMA_E1_DDALCU_CANDIDATE_3_READ_ONLY_LIVE_STATE_PREFLIGHT")
    if authorization.get("executionIdentity") != EXECUTION_IDENTITY or authorization.get("mode") != "MEASURED" or authorization.get("singleUse") is not True:
        raise RuntimeError("candidate-3 measured authorization scope mismatch")
    ...
    if identity.get("executionIdentity") != EXECUTION_IDENTITY or identity_acceptance.get("status") != "ACCEPTED_FROZEN_EXECUTION_IDENTITY":
        raise RuntimeError("candidate-3 execution identity is not accepted/frozen")
    if authorization.get("executionDriverSha256") != file_sha256(Path(__file__).resolve()):
        raise RuntimeError("candidate-3 execution driver drift")
    ...
    try:
        ...
        if len(plan) != 96:
            raise RuntimeError(f"candidate-3 frozen plan has {len(plan)} operations, expected 96")
    ...
    receipt = {
        "schemaVersion": "1.0.0",
        "recordType": "SMA_E1_DDALCU_CANDIDATE_3_MEASURED_EXECUTION_RECEIPT",
        "status": status,
