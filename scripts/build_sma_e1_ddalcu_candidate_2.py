...
SOURCE_LAUNCH = ROOT / "investigations/sma-q1/layered/sma-e1-ddalcu-candidate-1/e1/live-launch-definition.json"
PACKAGE_ROOT = ROOT / "investigations/sma-q1/layered/sma-e1-ddalcu-candidate-2/e1_candidate2"
LAUNCH = PACKAGE_ROOT / "live-launch-definition.json"
QUALIFICATION = ROOT / "OUTPUT/phase-3/sma-e1-ddalcu-candidate-2-offline-qualification/offline-qualification.json"
...
def build_launch() -> None:
    if LAUNCH.exists():
        raise RuntimeError("candidate-2 launch definition already exists")
    value = json.loads(SOURCE_LAUNCH.read_text(encoding="utf-8"))
    value["recordType"] = "SMA_E1_DDALCU_CANDIDATE_2_LIVE_LAUNCH_DEFINITION"
    disposable = value["disposable"]
    disposable.update({
        "mongoDatabase": "sma_e1_ddalcu_candidate_2",
        "semanticCollection": "sma_e1_ddalcu_candidate_2_semantic",
        "episodicCollection": "sma_e1_ddalcu_candidate_2_episodic",
        "workspaceRoot": "/Users/paul/work/tekroo-ai/sma-step15-s1-p2final/target/sma-e1-ddalcu-candidate-2",
        "launchLabel": "com.tekroo.sma-service-e1-ddalcu-candidate-2",
        "stubLaunchLabel": "com.tekroo.sma-e1-model-audit-proxy-candidate-2",
    })
    runtime = "/Users/paul/work/tekroo-ai/sma-step15-s1-p2final/target/sma-e1-ddalcu-candidate-2/runtime"
    value["evidenceFiles"] = {
    ...
    additions = [
        ...
        ROOT / "scripts/build_sma_e1_ddalcu_candidate_2.py",
        ROOT / "scripts/qualify_sma_e1_ddalcu_candidate_2.py",
    ]
    value["boundFiles"].extend({"path": str(path), "sha256": sha(path)} for path in additions)
...
def build_identity() -> None:
    if PACKAGE_IDENTITY.exists():
        raise RuntimeError("candidate-2 package identity already exists")
    if not LAUNCH.is_file() or not QUALIFICATION.is_file():
        raise RuntimeError("candidate-2 launch or offline qualification is missing")
    ...
    subject = {
        "lineage": "SMA-E1-DDALCU-CANDIDATE-2",
    ...
    value = {
        "schemaVersion": "1.0.0",
        "recordType": "SMA_E1_DDALCU_CANDIDATE_2_PACKAGE_IDENTITY",
