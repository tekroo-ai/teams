...
def main() -> int:
    ...
    run_id = "sma-s1-handler-implementation-check-p2m-candidate-7"
    namespaces = {
        "mongodbDatabase": "sma_s1_driver_fixture_handler_p2m_candidate_7",
        "semanticCollection": "sma_s1_driver_fixture_handler_semantic_p2m_candidate_7",
        "episodicCollection": "sma_s1_driver_fixture_handler_episodic_p2m_candidate_7",
        "actorAlphaPartition": "sma-s1-fixture-actor-alpha-handler-p2m-candidate-7",
        "actorBetaPartition": "sma-s1-fixture-actor-beta-handler-p2m-candidate-7",
        ...
        if prepare_pass:
            for case in manifest["cases"]:
                case_request = make_request(
                    ...
                    {"predicates": case["predicates"], "syntheticCorpus": manifest["syntheticCorpus"]},
                    run_id, manifest_hash, identity_hash, namespaces)
                response, error = invoke_and_record(journal, case_request,
                        "SMA_S1_HANDLER_CHECK_PREASSERTION")
                mechanics_pass = error is None and response is not None \
                        and response["terminalState"] == "SUCCEEDED" \
