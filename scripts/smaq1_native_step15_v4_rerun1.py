#!/usr/bin/env python3
"""Execute corrective rerun 1 of the frozen SMA-Q1 Step 15 v4 corpus.
...
MANIFEST = TEAMS / "investigations/sma-q1/preregistration-native-v4.json"
BASE_MANIFEST = TEAMS / "investigations/sma-q1/preregistration-native-v2.json"
IDENTITY = TEAMS / "investigations/sma-q1/step-15-execution-identity-v4-rerun1.json"
AUTHORIZATION = TEAMS / "investigations/sma-q1/step-15-execution-authorization-v4-rerun1.json"
CHILD_TOOL = TEAMS / "investigations/sma-q1/launch-child-conversation-client-tool.json"
V3_JOURNAL = TEAMS / "OUTPUT/phase-3/sma-q1n-step15-v3-rerun1/raw-receipts.jsonl"
OUTPUT = TEAMS / "OUTPUT/phase-3/sma-q1n-step15-v4-rerun1"
ORIGINAL_V4_OUTPUT = TEAMS / "OUTPUT/phase-3/sma-q1n-step15-v4"
RUN_ROOT = SMA / "target/sma-q1n-step15-20260813-v4"
...
LABEL = "com.tekroo.sma-service-smaq1n-step15-v4"
DATABASE = "sma_q1n_step15_20260813_v4"
SEMANTIC = "sma_q1n_step15_v4_semantic_20260813"
EPISODIC = "sma_q1n_step15_v4_episodic_20260813"
QUAL_RUN_ROOT = SMA / "target/sma-q1n-step15-20260813-v4-fixture"
QUAL_DATABASE = "sma_q1n_step15_v4_fixture_20260813"
QUAL_SEMANTIC = "sma_q1n_step15_v4_fixture_semantic_20260813"
QUAL_EPISODIC = "sma_q1n_step15_v4_fixture_episodic_20260813"
QUAL_LABEL = "com.tekroo.sma-service-smaq1n-step15-v4-fixture"
MANIFEST_SHA = "2178116c866e9abed9b5e83edd3c9c221f55f81e6f343a168205209f25834d23"
BASE_MANIFEST_SHA = "c618371d571d5333aebd2bdc83d2db2559115f5ec3e1903b33e8e0ab157edf5d"
V3_JOURNAL_SHA = "ff7817e4123aa784ad3bb09b4a9df13b35094919f050f273fb3069d7e5e743b8"
INTAKE_SOURCE = SMA / "src/main/java/ai/tekroo/sma/openhands/OpenHandsEventIntakeService.java"
REPLAY_SOURCE = SMA / "src/main/java/ai/tekroo/sma/replay/ReplayWorker.java"
VECTOR_SOURCE = SMA / "src/main/java/ai/tekroo/sma/vector/QdrantVectorStore.java"
INTAKE_SOURCE_SHA = "21a0d02ff42f42351c3a90c49118ca5c7c35fa67947fd7a2d2498f17b99a7fbc"
REPLAY_SOURCE_SHA = "36b9452cf9f7ed4586c6ac0c9ad6b6d2ea201f02788c4eea3766be43f0cf5773"
VECTOR_SOURCE_SHA = "ebde338949a6267809f35712ed7fd10267d6f95aae0fb1232a079c7134e32e93"
...
ALPHA_PARTITION = "openhands:" + hashlib.sha256(str(ALPHA).encode()).hexdigest()[:24] + ":default"
BETA_PARTITION = "openhands:" + hashlib.sha256(str(BETA).encode()).hexdigest()[:24] + ":default"
...
def workspace_partition(path: Path) -> str:
    """Match the pinned intake service's 24-hex canonical-workspace identity."""
    canonical = str(path.absolute()).replace("\\", "/").rstrip("/")
    return "openhands:" + hashlib.sha256(canonical.encode()).hexdigest()[:24] + ":default"


def retained_v3_qualification_observation() -> dict[str, Any]:
    matches = []
    with V3_JOURNAL.open(encoding="utf-8") as stream:
        for line in stream:
            record = json.loads(line)
            if record.get("kind") == "QUALIFICATION_CORPUS_OBSERVED":
                matches.append(record)
    if len(matches) != 1:
        raise RuntimeError(f"retained v3 qualification receipt cardinality: {len(matches)}")
    return matches[0]


def static_expectation_checks() -> dict[str, Any]:
    """Validate corrected expectations without querying or mutating a live service."""
    source_hashes = {
        "intake": sha_file(INTAKE_SOURCE),
        "replay": sha_file(REPLAY_SOURCE),
        "vector": sha_file(VECTOR_SOURCE),
    }
    source_rules = {
        "workspace_fingerprint_24": (
            "sha256(canonicalWorkspace(workspaceDir)).substring(0, 24)"
            in INTAKE_SOURCE.read_text(encoding="utf-8")
        ),
        "episodic_qdrant_upsert": (
            "vectorStorePort.upsert(VectorCollection.EPISODIC"
            in REPLAY_SOURCE.read_text(encoding="utf-8")
        ),
        "canonical_update_semantic_only": (
            "updateCanonical(locked.id(), currentDocVersion, newCanonical, semanticRef, now)"
            in REPLAY_SOURCE.read_text(encoding="utf-8")
        ),
    }
    expected_partitions = {
        "qualification_alpha": "openhands:4c95b7daff6686d709f08cfc:default",
        "qualification_beta": "openhands:78dfa54e08fa120a30768237:default",
        "measured_alpha": "openhands:5836d69e6762db4d4baf87cc:default",
        "measured_beta": "openhands:f7a4c51724a73ed846a7d4dc:default",
    }
    computed_partitions = {
    ...
    observation = retained_v3_qualification_observation()
    v3_root = SMA / "target/sma-q1n-step15-20260813-v3-fixture"
    v3_agents = {
        "alpha": workspace_partition(v3_root / "actor-alpha"),
        "beta": workspace_partition(v3_root / "actor-beta"),
    }
    corrected_problems: list[str] = []
    superseded_full_hash_rejections = 0
    superseded_episodic_ref_rejections = 0
    for logical_id, state in observation["states"].items():
        projection = observation["projections"][logical_id]
        eligible = bool(projection["eligible"])
        expected_agent = v3_agents["beta" if logical_id == "mem-beta-port" else "alpha"]
        expected_point_id = java_name_uuid(state["memory_id"])
        if state["agent_id"] != expected_agent:
            corrected_problems.append(f"{logical_id}:agent")
        if state["state"] != ("consistent" if eligible else "raw"):
            corrected_problems.append(f"{logical_id}:state")
        if bool(state["reasoning_eligible"]) != eligible:
            corrected_problems.append(f"{logical_id}:eligibility")
        if eligible and (state["semantic_ref"] != expected_point_id
                         or state["episodic_ref"] is not None):
            corrected_problems.append(f"{logical_id}:mongo_refs")
        if not eligible and (state["semantic_ref"] is not None
                             or state["episodic_ref"] is not None):
            corrected_problems.append(f"{logical_id}:ineligible_mongo_refs")
        for collection in ("semantic", "episodic"):
            receipt = projection[collection]
            if eligible:
                ...
                if (receipt["http_status"] != 200 or receipt["point_count"] != 1
                    ...
                    corrected_problems.append(f"{logical_id}:{collection}_projection")
            elif receipt["http_status"] != 200 or receipt["point_count"] != 0:
                corrected_problems.append(f"{logical_id}:{collection}_ineligible_projection")
        full_hash_agent = "openhands:" + hashlib.sha256(
            str(v3_root / ("actor-beta" if logical_id == "mem-beta-port" else "actor-alpha")).encode()
        ).hexdigest() + ":default"
        if state["agent_id"] != full_hash_agent:
            superseded_full_hash_rejections += 1
        if eligible and state["episodic_ref"] is None:
            superseded_episodic_ref_rejections += 1

    checks = {
        "source_hashes": source_hashes,
        "source_hashes_match": (
            source_hashes["intake"] == INTAKE_SOURCE_SHA
            and source_hashes["replay"] == REPLAY_SOURCE_SHA
            and source_hashes["vector"] == VECTOR_SOURCE_SHA
        ),
        "source_rules": source_rules,
        "v3_journal_sha256": sha_file(V3_JOURNAL),
        "v3_journal_matches": sha_file(V3_JOURNAL) == V3_JOURNAL_SHA,
        "expected_v4_partitions": expected_partitions,
        "computed_v4_partitions": computed_partitions,
        "v4_partitions_match": computed_partitions == expected_partitions,
        "v3_corrected_expectation_problems": corrected_problems,
        "v3_corrected_expectations_pass": not corrected_problems,
        "superseded_full_hash_rejections": superseded_full_hash_rejections,
        "superseded_episodic_ref_rejections": superseded_episodic_ref_rejections,
        "discriminates_both_corrections": (
            superseded_full_hash_rejections == 4
            and superseded_episodic_ref_rejections == 3
        ),
    }
    required = [
        checks["source_hashes_match"], all(source_rules.values()),
        checks["v3_journal_matches"], checks["v4_partitions_match"],
        checks["v3_corrected_expectations_pass"], checks["discriminates_both_corrections"],
    ]
    if not all(required):
        raise RuntimeError(f"static expectation self-test failed: {checks}")
    return checks
...
def service_environment(
    ...
    return {
        ...
        "SMA_CANARY_PARTITION": replay_canary or ALPHA_PARTITION,
...
def start_service(
    ...
    support.write_plist(plist, service_environment(capture, retrieval, replay_canary),
    ...
    return {
        "capture": capture,
        "retrieval": retrieval,
        "replay_canary": replay_canary or ALPHA_PARTITION,
        "startup": result,
        "health": health,
    }
...
def seed_corpus(corpus: list[dict[str, Any]], conversations: list[str],
    ...
    # Corpus construction owns replay order. A nonmatching canary keeps the
    # always-on background scheduler from racing explicit eligible promotions.
    journal.append(f"{receipt_prefix}SERVICE_START", start_service(
        True, True, "smaq1:controlled-corpus-construction",
    ))
    wait_memory_count(4)
    ...
    for item in corpus:
        ...
        if item["eligible"] and (state["episodic_ref"] is not None
...
def preflight(journal: DurableJournal) -> dict[str, Any]:
    ...
    checks = {
        ...
        "static_expectation_self_test_required": True,
        "corrective_rerun": {
            "authorized_by": "frozen v4 rerunPolicy",
            "attempt": 1,
            "classification": "CONTROLLED_CORPUS_BACKGROUND_REPLAY_ISOLATION_DEFECT",
            "original_harness": "scripts/smaq1_native_step15_v4.py",
            "original_harness_sha256": "5175b053f3633744212923290b0ac2614412a516e9279649b166d1ab2957b3e2",
            "original_receipt": str(ORIGINAL_V4_OUTPUT / "execution-receipt.json"),
            "original_receipt_sha256": "4f1f33d90f2d358d59ac8d1b161fc01904546248112e2846261d41ce5faaa492",
            "original_journal": str(ORIGINAL_V4_OUTPUT / "raw-receipts.jsonl"),
            "original_journal_sha256": "7fa6a99f7106203a332d8ed548f0b06437738504a42b5b4d48c05d71465ca868",
            "scenario_repetitions_started": 0,
            "correction": "Use a nonmatching replay canary only while constructing the controlled corpus; explicit eligible promotions and all frozen outcomes remain unchanged.",
        },
    }
    ...
    required = [
        ...
        checks["authorization_manifest_matches"],
        checks["authorization_scope_matches"], checks["scenario_count"] == 18,
        checks["repetition_count"] == 96, checks["identity_is_non_authorizing_fence"],
        checks["identity_freeze_accepted"],
...
def main() -> None:
    ...
    try:
        journal.append("STATIC_EXPECTATION_SELF_TEST", {
            "status": "PASS", **static_expectation_checks(),
        })
        preflight(journal)
