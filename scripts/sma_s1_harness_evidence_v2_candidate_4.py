...
HARNESS_VERSION = "2.0.0-evidence-complete-candidate-4"
RESTART_CASES = {
...
def minimum_evidence_gap(case_id: str, evidence: dict[str, Any]) -> str | None:
    ...
    if case_id == "SMA-S1-007-PARENT-CHILD-PROVENANCE":
        ...
        if (
            parent.get("parentConversationIdentity") != {"present": False}
            or parent.get("eventId") != "event-parent"
            or child.get("parentConversationIdentity")
            != {"present": True, "value": "conv-parent"}
        ...
        for conversation, parent, event, profile in (
            ...
            provenance.append({
                ...
                "parentConversationIdentity": (
                    {"present": False}
                    if parent is None
                    else {"present": True, "value": parent}
                ),
                "eventId": event,
