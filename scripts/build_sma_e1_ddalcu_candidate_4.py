...
SOURCE_LAUNCH = ROOT / "investigations/sma-q1/layered/sma-e1-ddalcu-candidate-3/e1_candidate3/live-launch-definition.json"
PACKAGE_ROOT = ROOT / "investigations/sma-q1/layered/sma-e1-ddalcu-candidate-4/e1_candidate4"
LAUNCH = PACKAGE_ROOT / "live-launch-definition.json"
QUALIFICATION = ROOT / "OUTPUT/phase-3/sma-e1-ddalcu-candidate-4-offline-qualification/offline-qualification.json"
...
def build_launch() -> None:
    if LAUNCH.exists():
        raise RuntimeError("candidate-4 launch definition already exists")
    value = json.loads(SOURCE_LAUNCH.read_text(encoding="utf-8"))
    value["recordType"] = "SMA_E1_DDALCU_CANDIDATE_4_LIVE_LAUNCH_DEFINITION"
    value["disposable"].update({
        "mongoDatabase": "sma_e1_ddalcu_candidate_4",
        "semanticCollection": "sma_e1_ddalcu_candidate_4_semantic",
        "episodicCollection": "sma_e1_ddalcu_candidate_4_episodic",
        "workspaceRoot": "/Users/paul/work/tekroo-ai/sma-step15-s1-p2final/target/sma-e1-ddalcu-candidate-4",
        "launchLabel": "com.tekroo.sma-service-e1-ddalcu-candidate-4",
        "stubLaunchLabel": "com.tekroo.sma-e1-model-audit-proxy-candidate-4",
    })
    runtime = "/Users/paul/work/tekroo-ai/sma-step15-s1-p2final/target/sma-e1-ddalcu-candidate-4/runtime"
    value["evidenceFiles"] = {
    ...
    value["agentLoopAudit"] = {
        "exactModelCallCountRequired": False,
        "minimumCallsPerLogicalInteraction": 1,
        "maximumCallsPerLogicalInteraction": 12,
        "requestResponseIdentityPairingRequired": True,
        "originalFrozenTaskRequiredInEveryFollowupRequest": True,
    ...
    additions = [
        ...
        ROOT / "scripts/build_sma_e1_ddalcu_candidate_4.py",
        ROOT / "scripts/qualify_sma_e1_ddalcu_candidate_4.py",
    ]
...
def build_identity() -> None:
    if PACKAGE_IDENTITY.exists():
        raise RuntimeError("candidate-4 package identity already exists")
    if not LAUNCH.is_file() or not QUALIFICATION.is_file():
        raise RuntimeError("candidate-4 launch or offline qualification is missing")
    ...
    subject = {
        "lineage": "SMA-E1-DDALCU-CANDIDATE-4",
    ...
    value = {
        "schemaVersion": "1.0.0",
        "recordType": "SMA_E1_DDALCU_CANDIDATE_4_PACKAGE_IDENTITY",
